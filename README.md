...
# Создаем агента (по умолчанию использует proxyapi.ru)
agent = TextAgent()
...
- `__init__(api_key=None, base_url="https://api.proxyapi.ru/anthropic")` - Инициализация агента
...
- `print_history()` - Вывести историю диалога в читаемом формате
- `start_chat(system_prompt=None)` - Начать новый чат

## Особенности

- Поддержка режима диалога с сохранением контекста
- Использование модели Claude Sonnet 4.5 через прокси API
