...
def handle_from_country(message, user_id):
    ...
    if not currency:
        bot.send_message(
            message.chat.id,
            f"❌ К сожалению, валюта для страны '{country}' не найдена в списке поддерживаемых валют API.\n\n"
            f"💡 Попробуйте:\n"
            f"   • Написать название страны по-другому\n"
            f"   • Использовать английское название\n"
            f"   • Выбрать популярную страну\n\n"
            f"Примеры: Россия, Таиланд, США, Шри Ланка",
...
def show_trips_by_status(chat_id, user_id, status='active'):
    ...
    status_text = "Активные поездки" if status == 'active' else "Архив поездок"
    
    if not trips:
        empty_text = "активных поездок" if status == 'active' else "закрытых поездок"
        bot.send_message(
            chat_id,
            f"{status_emoji} {status_text}\n\n"
            f"У вас пока нет {empty_text}.",
            reply_markup=get_main_menu_keyboard()
        )
        return
    ...
    for trip in trips:
        ...
        status_icon = "✅ " if is_active_now else ""
    ...
    instruction = "Нажмите на поездку, чтобы сделать её активной" if status == 'active' else "Нажмите на поездку, чтобы возобновить её"
...
def handle_close_trip(chat_id, user_id, message_id, trip_id):
    ...
    if not trip or trip['user_id'] != user_id:
        bot.answer_callback_query(message_id, "❌ Ошибка")
        return
    ...
    bot.send_message(
        ...
        f"   • {format_amount(trip['current_balance_foreign'])} {trip['to_currency']}\n"
        f"   • {format_amount(trip['current_balance_home'])} {trip['from_currency']}\n\n"
        f"Вы можете найти её в архиве поездок.",
        reply_markup=get_main_menu_keyboard()
    )
...
def handle_reopen_trip(chat_id, user_id, message_id, trip_id):
    ...
    bot.send_message(
        chat_id,
        f"✅ Поездка возобновлена!\n\n"
        f"📍 {trip['trip_name']}\n"
        f"💰 Баланс:\n"
        f"   • {format_amount(trip['current_balance_foreign'])} {trip['to_currency']}\n"
        f"   • {format_amount(trip['current_balance_home'])} {trip['from_currency']}\n\n"
        f"Поездка снова активна!",
        reply_markup=get_main_menu_keyboard()
    )
...
def show_history(chat_id, user_id):
    ...
    history_text += "💰 Расходы по категориям:\n\n"
    ...
    for cat in grouped_expenses:
        ...
        keyboard.add(
            types.InlineKeyboardButton(
                f"{cat['category_icon']} {cat['category_name']} — Детали",
...
def show_category_expenses(chat_id, user_id, message_id, category_id):
    ...
    if not expenses:
        bot.answer_callback_query(message_id, "❌ Нет расходов в этой категории")
        return
    
    history_text = f"{category['icon']} {category['name']}\n"
    history_text += f"📍 {trip['trip_name']}\n\n"
    ...
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton("🔙 К категориям", callback_data="menu_history"))
...
def handle_category_selection(chat_id, user_id, message_id, category_id):
    ...
    if not state or state['state'] != STATE_WAITING_CATEGORY:
        bot.answer_callback_query(message_id, "❌ Ошибка: неверное состояние")
        return
    ...
    bot.edit_message_text(
        f"✅ Расход учтён!\n\n"
        f"{category['icon']} Категория: {category['name']}\n\n"
        f"💸 Потрачено:\n"
